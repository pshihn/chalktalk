...
Sketch.prototype = {
   ...
   modelBeginFrame : function() {
      if (this._isModel) {
         ...
         if (! this.model()._sc0) {
            ...
            for (var i = 0 ; i < this.sketchTrace.length ; i++) {
               var strokeBounds = computeCurveBounds(this.sketchTrace[i]);
               xlo = min(xlo, strokeBounds.xlo);
               xhi = max(xhi, strokeBounds.xhi);
            }
            this.model()._tx0 = this.tx();
            this.model()._ty0 = this.ty();
            this.model()._sc0 = (xhi - xlo - sketchPadding) / width();
         }
         var bg = backgroundColor == 'black' ? [0,0,0] : [1,1,1];
         var tt = this.model()._sc0 / 0.142;
         var rx = this.rX, ry = this.rY, yy = min(1, 4 * ry * ry);
         this.model().color(mix(bg, palette.getRGB(this.colorId), this.getAlpha()));
         if (this._custom_model_xform === undefined) {
            this.model().xform(CT.matrixMultiply(CT.matrixTranslated(
                       (mix(this.model()._tx0, this.tx(), tt) + _g.panX) / (width() / 2),
                      -(mix(this.model()._ty0, this.ty(), tt) + _g.panY) / (width() / 2), 0),
                   ...
                   CT.matrixMultiply(CT.matrixScaled    (this.scale() * this.model()._sc0),
   ...
   modelEndFrame : function() {
      if (this._called_beginFrame) {
         if (this.inValue[0] && typeof this.inValue[0] != 'function')
   ...
   doCmdSwipe : function(i) {
      if (this.cmdSwipe[i] !== undefined && this.cmdSwipe[i] != null) {
         m.save();
         computeStandardViewInverse();
         this.computeCursorPoint(sketchPage.xDown, sketchPage.yDown);
         this._callbackFunction = this.cmdSwipe[i][1];
   ...
   doSwipe : function(i) {
      if (this.onSwipe[i] !== undefined && this.onSwipe[i] != null) {
         m.save();
         computeStandardViewInverse();
         this.computeCursorPoint(sketchPage.xDown, sketchPage.yDown);
         this._callbackFunction = this.onSwipe[i][1];
         this._callbackFunction();
         m.restore();
         return true;
      }
   ...
   xyToSketchPoint : function(x, y) {
      return this._mInverse.transform([this.unadjustX(x),this.unadjustY(y)]);
   },
   ...
   setText : function(text) {
      ...
      if (! this.isFreehandSketch() && ! (this instanceof Sketch2D))
         return;
   ...
   spliceRender : function(start, deleteCount, code) {
      ...
      this.render = Utility.spliceFunction(this.render, 2, 0, code);
...
// THIS IS WHERE TO PUT $1 CONSTRUCT.
   ...
   morphToGlyphSketch : function() {
      ...
      if (this._isModel)
         color(fadedColor(1 - t, preglyphSketch.colorId));
      ...
      _g.lineWidth = sketchLineWidth * mix(1, this._isModel ? 0 : .6, t);
      drawTrace(C);
   }
