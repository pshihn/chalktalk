...
///////// SAVING / LOADING THE PAGE TO / FROM THE SERVER //////////
      ...
      serialize : function () {
         ...
         for (n = 0 ; n < nsk() ; n++) {
            ...
            var copy = Utility.copyWithoutFunctions(sk(n));
	...
	var stringifiedData = Utility.stringify(sketchData);
...
///////////////////// EVENT HANDLERS /////////////////////////
	 ...
	 case 'spc': // Space bar toggles tool tips.
	    ...
	    break;
         ...
         case 'F':
            isFog = ! isFog;
            backgroundColor = isFog ? 'rgb(24,43,62)' : defaultPenColor == 'black' ? 'white' : 'black';
      ...
      animate : function(elapsed) {
         ...
         if (! this.isCTObject) {
            ...
            if (! this.ctObject) {
               ...
               this.ctObject.addChild(new CT.Cube());
               this.ctObject.addChild(new CT.Cylinder(8));
               this.ctObject.addChild(new CT.Extruded(16,100,
               ...
               var N = new Noise();
               this.ctObject.addChild(new CT.Parametric(20,20,function(u,v){return[2*u-1,2*v-1,.5*N.noise([3*u,3*v,.5])];}));
               var spline = makeSpline([[.001,-1],[.5,-1],[.45,-.95],[.15,-.9],[.09,-.85],[.05,-.6],[.1,0],
                                        [.2,.1],[.4,.2],[.6,.5],[.5,.98],[.48,1],[.46,.98],[.55,.5],[.35,.3],[.001,.2]]);
               this.ctObject.addChild(new CT.Revolved(16, 64, function(t){return sample(spline,t);}));
               this.ctObject.addChild(new CT.Sphere(16, 8));
               this.ctObject.addChild(new CT.Square());
               this.ctObject.addChild(new CT.Torus(32, 16, .6));
               for (var i = 0 ; i < this.ctObject.numChildren() ; i++) {
                  var C = palette.rgb[i];
...
// DRAW ALL THE SKETCHES.
         ...
         else if (isShowingHelp && !palette.isPieMenu) {
            drawToolTips();
         }
         else {
...
var isDrawingSketch2D = false;            // g sketchPage
...
var isShowingOverview = false;            // overview sketchPage
